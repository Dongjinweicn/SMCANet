...
def convert_targets(targets, device):
    ...
    shp_y = target_masks.shape
    target_masks = target_masks.long()
    y_onehot = torch.zeros((shp_y[0], num_class, shp_y[2], shp_y[3]))
    if target_masks.device.type == "cuda":
        y_onehot = y_onehot.cuda(target_masks.device.index)
    y_onehot.scatter_(1, target_masks, 1).float()
    target_masks = y_onehot
...
def convert_targets_2(targets, device):
    num_class = 6
    masks = [t["masks"] for t in targets]
    target_masks = torch.stack(masks)
    shp_y = target_masks.shape
    ...
    if target_masks.device.type == "cuda":
        y_onehot = y_onehot.cuda(target_masks.device.index)
    y_onehot.scatter_(1, target_masks, 1).float()
    target_masks_B = y_onehot

    masks_C = [t["masksC"] for t in targets]
    target_masks_C = torch.stack(masks_C)
    shp_y_C = target_masks_C.shape
    target_masks_C = target_masks_C.long()
    y_onehot_C = torch.zeros((shp_y_C[0], num_class, shp_y_C[2], shp_y_C[3]))
    if target_masks_C.device.type == "cuda":
        y_onehot_C = y_onehot_C.cuda(target_masks_C.device.index)
    y_onehot_C.scatter_(1, target_masks, 1).float()
    target_masks_C = y_onehot_C

    target_masks = {'centerB': target_masks_B, 'centerC': target_masks_C}
    return target_masks
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for step in range(total_steps):
        ...
        ##
        second = task if model.args.model in ['Unet', 'SMCAnet', 'Baseline'] else targets_onehot
...
def evaluate(model, criterion, postprocessors, dataloader_dict, device, output_dir, visualizer, epoch, writer):
    ...
    for step in range(total_steps):
        ...
        itertime = time.time() - start
        metric_logger.log_every(step, total_steps, datatime, itertime, print_freq, header)
        if step % round(total_steps / 4.) == 0:
            ##original  
            sample_list.append(samples.tensors[0])
            ##
            # a = outputs['pred_masks']
            # b = outputs['pred_masks'][0]

            # pre=torch.cat([outputs["pred_masks_q"],outputs["pred_masks_k"]], dim=0)
            pre=outputs["pred_masks_q"]
            _, pre_masks = torch.max(pre[0], 0, keepdims=True)  #xiugai0718
    ...
    if model.args.model in ['SMCAnet', 'Baseline']:
        ...
        # writer.add_scalar('avg_loss_loss_Inclusive', stats['loss_Inclusive'], epoch)
